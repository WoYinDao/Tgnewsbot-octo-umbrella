...
做交易的盯财经、做游戏的盯游戏、做开发的盯科技。把行业相关频道设为源，再改一下 `classifier.py` 里的 `RULE_KEYWORDS` 关键词表（比如加上你关注的公司名、币种、技术栈），就能把泛新闻流变成定制情报流。配合置信度阈值，噪音不会弹到你脸上。

### 👥 社群/团队资讯站：自动运营一个内容频道

给你的群友、同事或订阅者运营一个资讯频道，但不想每天手动搬运？把机器人挂在服务器上，它 24 小时自动选料、分类、写摘要、排版发布，每条快讯还带原文链接。你只需要偶尔调整一下源频道列表。

### ✍️ 内容创作素材库：选题不再靠翻聊天记录

做自媒体、写周报、做播客的，每天的日报就是现成的选题清单。所有消息连同分类、摘要、原文链接都存在本地 SQLite（`news.db`）里，想回溯某个事件的时间线，一条 SQL 就能把相关消息全拉出来。

### 🔕 免打扰阅读：把推送主动权拿回来

不想被消息轰炸但又怕错过大事？把快讯阈值（`AI_CONFIDENCE_THRESHOLD`）调高到 0.85，只有分类非常明确的大消息才会实时推送，其他全部攒到晚上日报一次性看。白天专心干活，晚上五分钟补完全天资讯。

> 💡 这些场景都不需要改核心代码：源频道、采集频率、快讯阈值、日报时间都在 `.env` 里配；只有自定义关键词需要改 `classifier.py` 里的一个字典。
