...
async def verify_bot_config():
    ...
    try:
        ...
        try:
            chat = bot.get_chat(chat_id=TARGET_CHAT_ID)
            print(f"✅ 目标频道访问成功")
...
async def verify_ai_config():
    ...
    try:
        ...
        print("\n✅ AI 配置正确")
        print(f"   注意：当前使用 openai 0.10.5，推荐使用 {OPENAI_MODEL} 模型")
...
async def main():
    ...
    print("\n" + "=" * 60)
    if all(results):
        print("🎉 所有配置验证通过！可以运行 python app.py 启动机器人")
    else:
